...
const createAccess = async (req, res, next) => {
  try {
    const { user_id, module_id, action_id, status } =
      req.body
...
const getAccessUser = async (req, res, next) => {
  try {
    ...
    const fathers = await conn.query("SELECT DISTINCT(module.id) AS id, module.name, module.url, mu.module_id " +
      "FROM accesos mu " +
      "RIGHT OUTER JOIN modules module ON module.id = mu.module_id AND mu.user_id = '"+userId+"'" +
    ...
    */
    const children = await conn.query("SELECT DISTINCT(module.id) AS id, module.name, module.url, module.icon, mu.module_id, module.module_id AS father_id " +
      "FROM accesos mu " +
      "RIGHT OUTER JOIN modules module ON module.id = mu.module_id AND mu.user_id = '"+userId+"'" +
    ...
    */
    const actions = await conn.query("SELECT DISTINCT(action.id), mu.action_id, module.id AS module_id, action.name, action.action_param, action.onclick, action.icon " +
      "FROM accesos mu " +
      "INNER JOIN modules module ON module.id = mu.module_id " +
      "RIGHT OUTER JOIN actions action ON action.id = mu.action_id AND mu.user_id = '"+userId+"' ",
      { type: QueryTypes.SELECT });
    ...
    */
    const actions_modules = await conn.query("SELECT action.id, "+'"moduleId"'+" AS module_id, action.name, action.action_param, action.onclick, action.icon " +
