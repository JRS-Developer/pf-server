...
const get_homeworkgrades = async(req, res, next)=>{
    try{
        ...
        const { error } = getHomeworkGradeSchema.validate({homeworkgrade_id, value})
...
const create_homeworkgrade = async(req, res, next)=>{
    try{
        ...
        const { error } = createHomeworkGradeSchema.validate({ value, homeworkgrade_id  })
        if (error) return res.status(400).json({ error: error.details[0].message })
        
        const homeworkgradeFound = await HomeworkGrade.findByPk(homeworkgrade_id)
        if (!homeworkgradeFound)return res.status(400).json({ error: 'There is not any homeworkgrade' })
